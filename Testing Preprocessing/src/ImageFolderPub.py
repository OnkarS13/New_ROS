...
class ImageLoad(object):
    def __init__(self):
        ...
        # Publishers for four different topics corresponding to positions P1, P2, P3, P4
        ...
        self.pub_p4 = rospy.Publisher('/lane_1/position_p4/image', Image, queue_size=1)
        
        self.rate = rospy.Rate(1)
        self.bridge = CvBridge()

        # Load images from folders A, B, C, D
        ...
        self.imgs_d = sorted(glob.glob("/home/agrograde/agrograde_ws/src/multilane_sorter/assets/images/lane_1/D/*.jpg"))
    ...
    def image_loop(self): 
        while not rospy.is_shutdown():
            try:
                ...
                # Fourth publish: from folder D (P4), C (P3), B (P2), and A (P1)
                if self.index < len(self.imgs_d):
                    img_p4 = io.imread(self.imgs_d[self.index])
                    img_p4_msg = self.bridge.cv2_to_imgmsg(img_p4, encoding='rgb8')
                    self.pub_p4.publish(img_p4_msg)
                    rospy.loginfo("Published image from folder D (P4)")

                if self.index + 1 < len(self.imgs_c):
                    img_p3 = io.imread(self.imgs_c[self.index + 1])
                    img_p3_msg = self.bridge.cv2_to_imgmsg(img_p3, encoding='rgb8')
                    self.pub_p3.publish(img_p3_msg)
                    rospy.loginfo("Published image from folder C (P3)")

                if self.index + 2 < len(self.imgs_b):
                    img_p2 = io.imread(self.imgs_b[self.index + 2])
                    img_p2_msg = self.bridge.cv2_to_imgmsg(img_p2, encoding='rgb8')
                    self.pub_p2.publish(img_p2_msg)
                    rospy.loginfo("Published image from folder B (P2)")

                if self.index + 3 < len(self.imgs_a):
                    img_p1 = io.imread(self.imgs_a[self.index + 3])
                    img_p1_msg = self.bridge.cv2_to_imgmsg(img_p1, encoding='rgb8')
                    self.pub_p1.publish(img_p1_msg)
                    rospy.loginfo("Published image from folder A (P1)")
                ...
                if (self.index >= len(self.imgs_a) or 
                    self.index >= len(self.imgs_b) or 
                    self.index >= len(self.imgs_c) or 
                    self.index >= len(self.imgs_d)):
                    self.index = 0
